"""
Script de fusion chronologique des données SOL de 2021 à 2024
...
def merge_sol_data():
    """Fusionne les données SOL de 2021 à 2024"""
    ...
    print("=" * 50)
    print("FUSION DES DONNÉES SOL (2021-2024)")
    print("=" * 50)
    
    for year in years:
        filepath = os.path.join(base_dir, f"SOL{year}.parquet")
        
        if os.path.exists(filepath):
            df = load_and_rename(filepath)
            print(f"✓ SOL{year}.parquet chargé : {len(df):,} lignes")
            dataframes.append(df)
        else:
            print(f"✗ SOL{year}.parquet non trouvé")
    ...
    # Sauvegarde
    output_path = os.path.join(base_dir, "SOL_merged_2021_2024.parquet")
    merged_df.to_parquet(output_path, index=False)
    print(f"\n✓ Fichier sauvegardé : SOL_merged_2021_2024.parquet")
