"""
Script de fusion chronologique des données ETH de 2021 à 2024
...
def merge_eth_data():
    """Fusionne les données ETH de 2021 à 2024"""
    ...
    print("=" * 50)
    print("FUSION DES DONNÉES ETH (2021-2024)")
    print("=" * 50)
    
    for year in years:
        filepath = os.path.join(base_dir, f"ETH{year}.parquet")
        
        if os.path.exists(filepath):
            df = load_and_rename(filepath)
            print(f"✓ ETH{year}.parquet chargé : {len(df):,} lignes")
            dataframes.append(df)
        else:
            print(f"✗ ETH{year}.parquet non trouvé")
    ...
    # Sauvegarde
    output_path = os.path.join(base_dir, "ETH_merged_2021_2024.parquet")
    merged_df.to_parquet(output_path, index=False)
    print(f"\n✓ Fichier sauvegardé : ETH_merged_2021_2024.parquet")
